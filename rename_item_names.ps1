...
$basePath = Join-Path $PSScriptRoot "Assets/Items"
if (-not (Test-Path $basePath)) {
    throw "No se encontró la carpeta Assets/Items en $basePath"
}
...
foreach ($setEntry in $setMappings.GetEnumerator()) {
    $setName = $setEntry.Key
    $names = $setEntry.Value

    if ($names.Count -ne 30) {
        throw "El set $setName no tiene 30 nombres (tiene $($names.Count))."
    }

    $index = 0
    foreach ($type in $typeOrder) {
        foreach ($level in $levels) {
            $filePath = Join-Path $basePath "$setName $type $level.asset"
            $desiredName = $names[$index]
            $index++
            ...
            for ($i = 0; $i -lt $content.Length; $i++) {
                ...
                if ($trimmed.StartsWith("itemName:")) {
                    $indent = $line.Substring(0, $line.Length - $trimmed.Length)
                    $content[$i] = "$indent" + "itemName: $desiredName"
                    $updated = $true
                    break
                }
            }
...
if ($missingFiles.Count -gt 0) {
    Write-Host "Archivos no encontrados:" -ForegroundColor Yellow
    $missingFiles | Sort-Object | ForEach-Object { Write-Host "  $_" }
}

if ($missingItemName.Count -gt 0) {
    Write-Host "itemName no encontrado en:" -ForegroundColor Yellow
    $missingItemName | Sort-Object | ForEach-Object { Write-Host "  $_" }
}

if ($missingFiles.Count -eq 0 -and $missingItemName.Count -eq 0) {
    Write-Host "Renombrado completado correctamente." -ForegroundColor Green
} else {
    Write-Host "Renombrado completado con advertencias." -ForegroundColor Yellow
}
